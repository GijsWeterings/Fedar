...
void pick(moveit::planning_interface::MoveGroup &group)
{
  ...
  geometry_msgs::PoseStamped qq = group.getCurrentPose();
  //group.setPoseTarget(qq);
  qq.pose.position.z = qq.pose.position.z + 0.01;
  group.setJointValueTarget("upper_arm_joint", 0.5);
  group.move();
}
...
//   g.retreat.desired_distance = 0.25;
//
//   g.post_place_posture.name.resize(1, "wrist_joint");
